...
1. 先看 [tsconfig.json 全配置详解](/posts/工程化/tsconfig全配置详解)，补齐 TypeScript 配置细节
2. 再看 [tsx / tsup / tsc 工具链对比](/posts/工程化/tsx-tsup-tsc工具链对比)，建立运行、检查、打包的职责边界
3. 再看 [环境变量与 dotenv](/posts/工程化/环境变量与dotenv)，把配置注入和类型校验补全
4. 接着看 [模块系统与路径别名](/posts/工程化/模块系统与路径别名)，把模块语义和路径管理理顺
5. 最后看 [OpenAI SDK 类型系统详解](/posts/工程化/OpenAI-SDK类型系统详解)，理解复杂 SDK 在 TypeScript 工程里的类型组织方式
